  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-2 sm:gap-0">
        <h1 className="text-xl sm:text-2xl font-bold text-gray-900">会社カレンダー</h1>
        <div className="text-xs sm:text-sm text-gray-500">休業日・繁忙日・特別営業日を管理</div>
      </div>

      {/* 凡例 */}
      <div className="bg-white rounded-lg shadow-sm border p-3 sm:p-4 mb-4 flex flex-wrap gap-2 sm:gap-3">
        {DAY_TYPES.map((t) => (
      ...
      {/* 年月選択 */}
      <div className="bg-white rounded-lg shadow-sm border p-3 sm:p-4 mb-4 flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4">
        <button
          onClick={() => { if (month === 1) { setYear(year - 1); setMonth(12); } else setMonth(month - 1); }}
          className="px-3 py-1 bg-gray-100 rounded hover:bg-gray-200 text-xs sm:text-sm"
        >◀</button>
        <select value={year} onChange={(e) => setYear(Number(e.target.value))} className="border rounded px-2 py-1 text-xs sm:text-sm">
          {[2025, 2026, 2027].map((y) => <option key={y} value={y}>{y}年</option>)}
        </select>
        <select value={month} onChange={(e) => setMonth(Number(e.target.value))} className="border rounded px-2 py-1 text-xs sm:text-sm">
        ...
        <button
          ...
          className="px-3 py-1 bg-gray-100 rounded hover:bg-gray-200 text-xs sm:text-sm"
        >▶</button>
        <span className="ml-0 sm:ml-4 text-xs sm:text-sm text-gray-500">
      ...
      <div className="bg-white rounded-lg shadow-sm border overflow-hidden">
        ...
        <div className="grid grid-cols-7 border-b">
          {WEEKDAY_LABELS.map((label, i) => (
            <div key={label} className={`py-1.5 sm:py-2 text-center text-xs sm:text-sm font-medium ${
        ...
        <div className="grid grid-cols-7">
          {days.map((day, idx) => {
            if (day === null) return <div key={idx} className="min-h-[60px] sm:min-h-[80px] bg-gray-50 border-b border-r" />;
            ...
            return (
              <div
                key={idx}
                className={`min-h-[60px] sm:min-h-[80px] border-b border-r p-1 cursor-pointer hover:bg-blue-50 transition ${
                  entry ? "bg-opacity-50" : ""
                }`}
      ...
      {selectedDate && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setSelectedDate(null)}>
          <div className="bg-white rounded-lg shadow-xl p-4 sm:p-6 w-full max-w-md mx-4" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-base sm:text-lg font-bold mb-3 sm:mb-4">{selectedDate} の設定</h3>
            <div className="space-y-2 sm:space-y-3">
              <div>
                <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">日程タイプ</label>
                <select
                  value={formType}
              ...
              <div>
                <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">備考</label>
                <input
                  value={formNote}
            ...
            <div className="flex flex-col sm:flex-row gap-2 mt-4 sm:mt-6">
              ...
              <button onClick={() => setSelectedDate(null)} className="flex-1 px-4 py-2 bg-gray-200 rounded text-sm hover:bg-gray-300">閉じる</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-lg shadow-sm border p-3 sm:p-4 mt-4">
        <h2 className="text-base sm:text-lg font-bold mb-3">{year}年 登録一覧</h2>
        {entries.length === 0 ? (
          ...
          <table className="w-full text-xs sm:text-sm">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-2 sm:px-3 py-1.5 sm:py-2 text-left">日付</th>
                <th className="px-2 sm:px-3 py-1.5 sm:py-2 text-left">タイプ</th>
                <th className="px-2 sm:px-3 py-1.5 sm:py-2 text-left">名称</th>
                <th className="px-2 sm:px-3 py-1.5 sm:py-2 text-left">備考</th>
              </tr>
            </thead>
            <tbody>
              {entries.map((e) => (
                <tr key={e.id} className="border-t hover:bg-gray-50">
                  <td className="px-2 sm:px-3 py-1.5 sm:py-2">{e.calendarDate.split("T")[0]}</td>
                  <td className="px-2 sm:px-3 py-1.5 sm:py-2">
                    <span className={`px-2 py-0.5 rounded text-xs ${getDayTypeInfo(e.dayType).color}`}>
                      {getDayTypeInfo(e.dayType).label}
                    </span>
                  </td>
                  <td className="px-2 sm:px-3 py-1.5 sm:py-2">{e.name || "-"}</td>
                  <td className="px-2 sm:px-3 py-1.5 sm:py-2">{e.note || "-"}</td>
                </tr>
              ))}
            </tbody>
          </table>
